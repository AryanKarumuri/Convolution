...
public:
    virtual ~miopen_convolution_impl_base_t() {
        std::cout<<__LINE__<<__FUNCTION__<<"Destroy Tensor Code Entry"<<std::endl;
       ...
       std::cout<<__LINE__<<__FUNCTION__<<"Destroy Tensor Code Exit"<<std::endl; 
    }
    ...
    void get_dims_and_strides(int io) {
        std::cout<<__LINE__<<" : value of io= "<<io<<std::endl;
        convert_dims(dnnl_descs[io].dims, dims[io], dnnl_descs[io].ndims, ndims[io]);

        std::cout<<__LINE__<<" dnnl_descs[io].ndims= "<<dnnl_descs[io].ndims<<std::endl;
        std::cout<<" ndims[io]= "<<ndims[io]<<std::endl;
        
        for(int i=0; i<dnnl_descs[io].ndims; i++){
                std::cout<<__LINE__<<" : "<<"dnnl_dims= "<<dnnl_descs[io].dims[i]<<std::endl;
            }
        for(int j=0; j<ndims[io]; j++){
            std::cout<<__LINE__<<" : "<<"dims[io][j]= "<<dims[io][j]<<std::endl;
        }
        if (ndims[io] > dnnl_descs[io].ndims) {
            std::cout<<__LINE__<<std::endl;
            std::cout<<__LINE__<<" : "<<"dim[io][4]= "<<dims[io][ndims[io] - 1]<<" dim[io][3]= "<<dims[io][ndims[io] - 2]<<std::endl;
            std::swap(dims[io][ndims[io] - 1], dims[io][ndims[io] - 2]);
            std::cout<<__LINE__<<" : "<<"dim[io][4]= "<<dims[io][ndims[io] - 1]<<" dim[io][3]= "<<dims[io][ndims[io] - 2]<<std::endl;
            if (ndims[io] == 4) {
                if (formats[io] == miopenTensorNHWC) {
                    std::cout<<__LINE__<<" :  I-strides= "<<strides[io][0]<<" "<<strides[io][1]<<" "<<strides[io][2]<<" "<<strides[io][3]<<std::endl;
                    propagate_strides(strides[io], dims[io], {1, 3, 2, 0});
                    std::cout<<__LINE__<<" : O-strides= "<<strides[io][0]<<" "<<strides[io][1]<<" "<<strides[io][2]<<" "<<strides[io][3]<<std::endl;
                } else {
                    std::cout<<__LINE__<<" : I-strides= "<<strides[io][0]<<" "<<strides[io][1]<<" "<<strides[io][2]<<" "<<strides[io][3]<<std::endl;
                ...
                /*for (int k = ndims[io] - 2; k >= 0; k--) {
                    strides[io][k] = (k != ndims[io] - 2) ? strides[io][k + 1] * dims[io][k + 2] : 1;
                }*/
            ...
            for(int i=0; i<dnnl_descs[io].ndims; i++){
                std::cout<<__LINE__<<" : "<<"dnnl_block_strides= "<<dnnl_descs[io].format_desc.blocking.strides[i]<<std::endl;
            }
            for(int j=0; j<ndims[io]; j++){
            std::cout<<__LINE__<<" : "<<"strides[io][j]= "<<strides[io][j]<<std::endl;
            }
            std::cout<<__LINE__<<std::endl;
    ...
    status_t configure_parameters(const convolution_pd_t *pd) {
        ...
        std::cout<<__LINE__<<" Printing the value of with_groups "<<std::endl;
        std::cout<<__LINE__<<" with_groups= "<< with_groups<<std::endl;

        std::cout<<__LINE__<<" Printing the value of ndims[weights] "<<std::endl;
        std::cout<<__LINE__<<" ndims[weights]= "<< ndims[weights]<<std::endl;
    ...
    status_t create_miopen_descs(const convolution_pd_t *pd) {
        ...
        for(int j=0; j<4; j++){
            std::cout<<__LINE__<<" : "<<"weight_strides[j]= "<<strides[weights][j]<<std::endl;
            }

        std::cout<<__LINE__<<" Printing the value of ndims[weights] "<<std::endl;
        std::cout<<__LINE__<<" : "<<" ndims[weights]= "<<ndims[weights]<<std::endl;
        std::cout<<__LINE__<<" : "<<" strides[weights][0]= "<<strides[weights][0]<<std::endl;
        std::cout<<__LINE__<<" : "<<" strides[weights][4]= "<<strides[weights][4]<<std::endl;

        std::cout<<__LINE__<<" Updated with_groups value :"<< with_groups<<std::endl;
...
public:
    ...
    void execute(miopenHandle_t handle,
        ...
        std::cout<<__LINE__<<" : "<<"x= "<<x<<" "<<"weights= "<<weights<<" "<<"y= "<<y<<std::endl;
    ...
    status_t init_scratchpad(engine_t *engine, convolution_pd_t *pd) override {
        ...
        ::std::cout << __LINE__ << " maxSolutionCount : " << maxSolutionCount << ::std::endl;
...
protected:
    ...
    // miopenConvBwdDataAlgorithm_t bwd_algo;
    //miopenConvBwdDataAlgorithm_t bwd_algo = miopenConvolutionBwdDataAlgoGEMM;
    //std::vector<miopenConvAlgoPerf_t> perf;
...
// ---------------------------------------------------------------------------------------
        ...
        for (int i=0; i < solutionCount; i++) {  // Added to check workspace is not excceding maximum size of MIOpen memory
            if (solutions[i].workspace_size >= MAX_MIOPEN_WK_SIZE)
                continue;
            selected_sol = i;
        }
        ...
        std::cout << __LINE__ << " ws_size : " << solutions[selected_sol].workspace_size << ::std::endl;
        std::cout << __LINE__ << " solution_id : " << solution_id << ::std::endl;
        std::cout<<__FILE__<<" : "<<__func__<<" : "<<__LINE__<<std::endl;
    ...
    void execute(miopenHandle_t handle,
        ...
        MIOPEN_EXECUTE_FUNC_S(miopenFindConvolutionBackwardDataAlgorithm,
                handle, descs[io::y], y, weights_desc, weights,
                conv_desc, descs[io::x],x, requested_algo_count,
                &returned_algo_count, &perf, &scratchpad, scratchpad_size,
                true); //changed       
        
        bwd_algo=perf.bwd_data_algo;
        std::cout<<__FILE__<<" : "<<__func__<<" : " << "bwd_algo: " << perf.bwd_data_algo <<__LINE__<<std::endl;
       ...
       MIOPEN_EXECUTE_FUNC_S(miopenConvolutionBackwardDataGetSolutionCount,handle,
                            descs[io::y],
                            weights_desc,
                            conv_desc,
                            descs[io::x],
        ...
        std::cout << __LINE__ << " solutionCount : " << solutionCount << ::std::endl;

        std::vector<miopenConvSolution_t> solutions(solutionCount);

        MIOPEN_EXECUTE_FUNC_S(miopenConvolutionBackwardDataGetSolution,handle, 
                            descs[io::y], 
                            weights_desc, 
                            conv_desc, 
                            descs[io::x],
        ...
        std::cout << __LINE__ << " solutions.data : " << solutions.data() << ::std::endl;

        auto ws_size = solutions[0].workspace_size;
        uint8_t *ws = nullptr;
        if(ws_size != 0){
        hipMallocManaged(&ws, ws_size, hipMemAttachGlobal);
        }

        std::cout<<__FILE__<<" : "<<__func__<<" : "<<__LINE__<<std::endl;
        auto solution_id = solutions[0].solution_id;
        
        MIOPEN_EXECUTE_FUNC_S(miopenConvolutionBackwardDataGetSolutionWorkspaceSize,handle, 
                            descs[io::y], 
                            weights_desc, 
                            conv_desc, 
                            descs[io::x], 
        ...
        MIOPEN_EXECUTE_FUNC_S(miopenConvolutionBackwardDataCompileSolution,handle,
                             descs[io::y], 
                             weights_desc, 
                             conv_desc, 
                             descs[io::x], 
        ...
        */
        ...
        MIOPEN_EXECUTE_FUNC_V(miopenConvolutionBackwardDataImmediate,handle,
				            ...
				            conv_desc,
                            descs[io::x],x,
                            scratchpad, ws_size,//scratchpad, scratchpad_size,
                            solutions[selected_sol].solution_id);

        std::cout << __LINE__ << " ws_size : " << solutions[selected_sol].workspace_size << ::std::endl;
        std::cout << __LINE__ << " solution_id : " << solutions[selected_sol].solution_id << ::std::endl;
...
public:
    ...
    status_t init_zero_dims(convolution_pd_t *pd) override {
        ...
        std::cout<<__LINE__<<" Printing the value of dnnl_descs[weights].ndims "<<std::endl;
        std::cout<<__LINE__<<" dnnl_descs[weights].ndims "<<dnnl_descs[weights].ndims<<std::endl;
        
        std::cout<<__LINE__<<" Printing the value of pd->with_groups() "<<std::endl;
        std::cout<<__LINE__<<" pd->with_groups() "<<pd->with_groups()<<std::endl;
    ...
    void execute(miopenHandle_t handle,
        ...
        */
        if (with_bias) {
            std::cout<<__FILE__<<" : "<<__func__<<" : "<<__LINE__<<std::endl;
            std::cout<<__FILE__<<" bias_alpha "<<*(&bias_alpha)<<" : "<<__LINE__<<std::endl;
            std::cout<<__FILE__<<" bias_beta "<<*(&bias_beta)<<" : "<<__LINE__<<std::endl;
